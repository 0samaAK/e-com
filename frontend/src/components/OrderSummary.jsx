...
const stripePromise = loadStripe("pk_test_51RMbTrFS6jF9syndrTQeNWvofAlCCt4XHADFumJOBNy0w4bqM9ZR8utNFC5plXHU73qQ6PNlcSUR4eyu9f8RFvUs00ibuCXYhn")
...
const OrderSummary = () => {
	...
	const handlePayment = async () => {
		...
		const res = await axios.post("http://localhost:3000/api/create-checkout-session", { products: cart, couponCode: coupon ? coupon.data[0].code : null }, { withCredentials: true })
				...
				<div className = 'space-y-2'>
...

					{
						coupon && isCouponApplied && (
							<dl className = 'flex items-center justify-between gap-4'>
								<dt className = 'text-base font-normal text-gray-300'>Coupon ({ coupon.data[0].code })</dt>
								<dd className = 'text-base font-medium text-emerald-400'>-{ coupon.data[0].discountPercentage }%</dd>
							</dl>
						)
        		  	}
