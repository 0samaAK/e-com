	...
	useEffect(() => {
		...
		const fetchAnalyticsData = async () => {
			...
			try {
				...
				const response = await axios.get("http://localhost:3000/api/analytics", { withCredentials: true })
				// setAnalyticsData(response.data.analyticsData)
				
				const { totalUsers, totalProducts, totalSales, totalRevenue } = response.data.analyticsData;
